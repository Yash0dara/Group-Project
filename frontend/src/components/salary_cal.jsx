...
const SalaryPage = () => {
  ...
  const handleSaveSalary = async (employeeId, salary) => {
    try {
      const response = await axios.post("http://localhost:8070/employees/save_salary", {
        salry_ID: employeeId,
      ...
      if (response.status === 200) {
        console.log(`Saved salary ${salary} for employee with ID ${employeeId}`);
        ...
        console.error("Failed to save salary:", response.data);
        // Handle error
      }
    } catch (error) {
